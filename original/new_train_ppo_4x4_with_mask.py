...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gym-id",
        type=str,
        default="MicrortsMining10x10F9-v0",
...
class MicroRTSStatsRecorder(gym.Wrapper):
    ...
    def step(self, action):
        observation, reward, done, info = super(MicroRTSStatsRecorder, self).step(
            action
        )
    ...
    def get_action(self, x, action=None, invalid_action_masks=None):
        ...
        if invalid_action_masks is not None:
            split_invalid_action_masks = torch.split(
                invalid_action_masks, envs.action_space.nvec.tolist(), dim=1
            )
            multi_categoricals = [
...
for update in range(1, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        invalid_action_masks[step] = torch.Tensor(
            np.array(envs.get_attr("action_mask"))
        )

        # ALGO LOGIC: put action logic here
        with torch.no_grad():
            values[step] = agent.get_value(obs[step]).flatten()
            action, logproba, _ = agent.get_action(
                obs[step], invalid_action_masks=invalid_action_masks[step]
            )
