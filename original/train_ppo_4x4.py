...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gym-id",
        type=str,
        default="MicrortsMining4x4F9-v0",
    ...
    parser.add_argument(
        "--batch-size", type=int, default=2048, help="the batch size of ppo"
    )
    parser.add_argument(
        "--minibatch-size", type=int, default=256, help="the mini batch size of ppo"
    )
    parser.add_argument(
...
assert isinstance(env, TimeLimit) or int(
    args.episode_length
), "the gym env does not have a built in TimeLimit, please specify by using --episode-length"
...
class Value(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(16 * 3 * 3, 128), nn.ReLU(), nn.Linear(128, 1)
        )
...
while global_step < args.total_timesteps:
    ...
    for step in range(args.batch_size):
        ...
        try:
            ...
            rewards[step] += (
                pd.DataFrame([info["invalid_action_stats"]]).sum().sum()
                * args.invalid_action_penalty
            )
        ...
        if dones[step]:
            ...
            writer.add_scalar(
                "charts/episode_reward", np.sum(real_rewards), global_step
            )
            print(f"global_step={global_step}, episode_reward={np.sum(real_rewards)}")
            real_rewards = []
            for key, idx in zip(
                info["invalid_action_stats"], range(len(info["invalid_action_stats"]))
            ):
                writer.add_scalar(
                    f"stats/{key}",
                    pd.DataFrame(invalid_action_stats).sum(0)[idx],
                    global_step,
                )
            invalid_action_stats = []
            next_obs = np.array(env.reset())
    ...
    if not dones[step]:
        last_value = (
            vf.forward(next_obs.reshape((1,) + next_obs.shape))[0]
            .detach()
            .cpu()
            .numpy()[0]
        )
    ...
    if args.gae:
        ...
        deltas = (
            bootstrapped_rewards[:-1]
            + args.gamma * bootstrapped_values[1:] * (1 - dones)
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            clip_adv = torch.where(
                advantages[minibatch_ind] > 0,
                (1.0 + args.clip_coef) * advantages[minibatch_ind],
                (1.0 - args.clip_coef) * advantages[minibatch_ind],
            ).to(device)
            ...
            policy_loss = (
                -torch.min(ratio * advantages[minibatch_ind], clip_adv)
            ...
            if args.clip_vloss:
                ...
                v_clipped = values[minibatch_ind] + torch.clamp(
                    new_values - values[minibatch_ind], -args.clip_coef, args.clip_coef
                )
        ...
        if args.kle_rollback:
            if (
                logprobs[:, minibatch_ind]
                - pg.get_action(
                    obs[minibatch_ind],
                    torch.LongTensor(actions[minibatch_ind].astype(np.int))
                    .to(device)
                    .T,
                )[1]
            ).mean() > args.target_kl:
    ...
    writer.add_scalar(
        "charts/policy_learning_rate", pg_optimizer.param_groups[0]["lr"], global_step
    )
    writer.add_scalar(
        "charts/value_learning_rate", v_optimizer.param_groups[0]["lr"], global_step
    )
